...
public:
  ...
  explicit SO3(const Eigen::MatrixBase<OtherDerived>& other)
      : SpecialOrthogonal<double, 3>(other) {}
  ...
  static Eigen::Vector3d error(const Eigen::MatrixBase<T1>& R, const Eigen::MatrixBase<T2>& Rd) {
    ...
    return (Eigen::Vector3d() << eR(2, 1), eR(0, 2), eR(1, 0)).finished();
  }
  template <typename T1, typename T2>
  ...
  Eigen::Vector3d error(const SO3& other) { return SO3::error(*this, other); }

  double config_error(const SO3& other) { return SO3::config_error(*this, other); }
