...
static const std::chrono::time_point<std::chrono::system_clock> NONLINEAR_CONTROLS_START_TIME =
...
inline Eigen::Matrix3d hat(const Eigen::Vector3d& vector_) {
  return (Eigen::Matrix3d() << 0.0, -vector_(2), vector_(1), vector_(2), 0.0, -vector_(0),
          -vector_(1), vector_(0), 0.0)
      .finished();
}

inline Eigen::Vector3d vee(const Eigen::Matrix3d& mat_) {
  return (Eigen::Vector3d() << mat_(2, 1), mat_(0, 2), mat_(1, 0)).finished();
}
...
inline Eigen::Vector3d rotmToZYXEulerAngles(const Eigen::Matrix3d& R) {
...
inline Eigen::Matrix<double, 4, 4> getTransformMat(const Eigen::Matrix3d& R,
