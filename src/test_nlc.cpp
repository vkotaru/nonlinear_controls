...
int main() {
    ...
    std::cout << "SO3d::error(R, Rd) = [ " << SO3d::error(R, Rd).transpose() << " ]^T" << std::endl;

    std::cout << "***************************************" << std::endl;
    TSE3<float> x, xd;

    const Eigen::Matrix<float, 3, 1> p = Eigen::Vector3f::Random();
    const Eigen::Matrix<float, 3, 1> v = Eigen::Vector3f::Random();
    const Eigen::Matrix<float, 3, 1> Om = Eigen::Vector3f::Random();
    const auto pd = Eigen::Vector3f::Ones();
    const auto vd = Eigen::Vector3f::Ones();
    const auto Omd = Eigen::Vector3f::Zero();
    ...
    xd.velocity = vd;
    xd.Omega = Omd;
    xd.R = Eigen::Matrix<float, 3, 3>::Identity();

    auto eR2_hat = 0.5 * (xd.R.transpose() * x.R - x.R.transpose() * xd.R);
    Eigen::Matrix<float, 3, 1> eR2;
    eR2 << eR2_hat(2, 1), eR2_hat(0, 2), eR2_hat(1, 0);

    Eigen::Matrix<float, 12, 1> outside_error;
