...
MPCBase::MPCBase(const bool _isLTI, const int _N, const int _nx, const int _nu)
...
void MPCBase::construct() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    // Xlb.block(nx * N, 0, nx, 1) << 1., 1., 1., 0., 0., 0.;
    // Xub.block(nx * N, 0, nx, 1) << 1., 1., 1., 0., 0., 0.;
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> MPCBase::update(
      const bool verbose, const Eigen::Matrix<double, Eigen::Dynamic, 1> &x0) {
