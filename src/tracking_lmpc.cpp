...
TrackingLinearMPC::TrackingLinearMPC(const int &_N, const int &_nx, const int &_nu) :
    N(_N), nx(_nx), nu(_nu) {
...
void TrackingLinearMPC::set_mpc_gains(const MatrixXd &_Q, const MatrixXd &_P, const MatrixXd &_R) {
...
void TrackingLinearMPC::set_input_bounds(const VectorXd &_ulb, const VectorXd &_uub) {
...
void TrackingLinearMPC::set_state_bounds(const VectorXd &_xlb, const VectorXd &_xub) {
...
void TrackingLinearMPC::set_lti_dynamics(const MatrixXd &_A, const MatrixXd &_B) {
...
void TrackingLinearMPC::construct() {
  ////////////////////////////////////////
...
void TrackingLinearMPC::run(const VectorXd &x0, const VectorXd &Xd) {
  assert(x0.rows() == nx && x0.cols() == nx);
  assert(Xd.rows() == nx && Xd.cols() == nu);
