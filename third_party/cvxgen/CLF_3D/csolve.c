/* Produced by CVXGEN, 2020-07-30 04:41:34 -0400.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 3))) {
      printf("A must be size (1,3), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 3) && (mxGetN(xm) == 1))) {
      printf("c must be size (3,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "xlb");
  if (xm == NULL) {
    printf("could not find params.xlb.\n");
  } else {
    if (!((mxGetM(xm) == 3) && (mxGetN(xm) == 1))) {
      printf("xlb must be size (3,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter xlb must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter xlb must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter xlb must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.xlb;
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "xub");
  if (xm == NULL) {
    printf("could not find params.xub.\n");
  } else {
    if (!((mxGetM(xm) == 3) && (mxGetN(xm) == 1))) {
      printf("xub must be size (3,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter xub must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter xub must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter xub must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.xub;
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 3; i++)
      printf("  params.xlb[%d] = %.6g;\n", i, params.xlb[i]);
    for (i = 0; i < 3; i++)
      printf("  params.xub[%d] = %.6g;\n", i, params.xub[i]);
  }
