...
/// trajectories
////////////////////////////////////////////
void sinusoidalTraj(double& yd, double& dyd, double& d2yd, const float t, const double freq,
                    const double amp, const double ampPercent, double ampOffset,
                    double freqOffset) {
  ...
  //    freqOffset);
  d2yd = 2 * amp * ampPercent * pow(freq, 2) * pow(sin(freqOffset + freq * t), 2) -
...
void test_position_mpc() {
  ...
  nu = 3;
  N = 10;
  nlc::MPCQPOases pos_mpc_(N, nx, nu);
  ...
  P.resize(nx, nx);
  R.resize(nu, nu);
  Q << 100 * Eigen::Matrix3d::Identity(), Eigen::Matrix3d::Zero(), Eigen::Matrix3d::Zero(),
