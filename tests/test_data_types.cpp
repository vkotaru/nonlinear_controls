...
protected:
  void SetUp() override {
    ...
    R << 0.8073, -0.5176, 0.2835, 0.5229, 0.8501, 0.0630, -0.2736, 0.0974, 0.9569;

    Rd << 0.4569, -0.0903, 0.8849, 0.7532, 0.5684, -0.3309, -0.4731, 0.8178, 0.3278;
  }
...
TEST_F(SO3Test, InverseEqualsTranspose) {
  ...
  EXPECT_LT((R_inv - R_trans).norm(), 1e-10) << "For SO3, inverse should equal transpose";
}
...
protected:
  void SetUp() override {
    ...
    x.R << 0.8073, -0.5176, 0.2835, 0.5229, 0.8501, 0.0630, -0.2736, 0.0974, 0.9569;
...
TEST_F(TSE3Test, ErrorComputationManual) {
  ...
  manual_error << (x.position - xd.position), (x.velocity - xd.velocity), eR,
